...
for comparative_idx in range(3):
    ...
    for i in [0]:
        ...
        for j in tqdm(range(100, len(X_test)-100, 10)):
            background_sample_idx = i+j
            prediction = first_output_model.predict(X_test[background_sample_idx][None, :])
            final_ref.append([prediction])
            k = 0
...
refs_plot = np.array(final_refs).squeeze()
for ref in refs_plot[0]:
    sum_till_ref = np.sum(ref[:])
